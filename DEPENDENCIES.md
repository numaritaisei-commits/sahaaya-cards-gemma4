...
| Keras and JAX | JAX 0.7.2 and Keras 3.13.2 in the official Kaggle T4 x2 image | Separate model-parallel structure, checkpoint, and bounded generation diagnostics; not the app | Upstream terms | Not included | JAX structure diagnostic Version 4 completed in 44.350s with `STRUCTURE_SHARDING_PASS`, 1,951 variables, 248 sharded variables, and `load_weights=False`; JAX weighted diagnostic Version 1 ended after 176.725s as `DIAGNOSTIC_FAILURE` / `VALUE` at `model_load_started` with `weights_loaded=false`; JAX weighted generation Version 3 used feature-column sharding and completed in 498.684s with `WEIGHTED_GENERATION_PASS`, all official weights restored and one bounded generation |
